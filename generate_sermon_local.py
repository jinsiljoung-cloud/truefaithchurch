...
# -*- coding: utf-8 -*-
"""
바른믿음교회 — 설교 자동생성 스크립트 (Ollama 로컬 버전)
API 키 불필요! Gemma 4 로컬에서 완전 무료!

실행: python generate_sermon_local.py [wednesday|sunday]
"""
...
# ══════════════════════════════════════════════
# 2) Ollama 로컬 AI로 설교 생성
# ══════════════════════════════════════════════
def generate_sermon_content(info):
    print(f"\n🤖 Ollama ({OLLAMA_MODEL}) 설교 생성 중...")
    print("   (첫 실행 시 모델 로딩에 30초~1분 소요됩니다)")

    prompt = f"""당신은 {CHURCH['name_ko']} 설교 작성 전문가입니다.

교회 정보: 표어={CHURCH['motto']}, 타겟={CHURCH['target']}, 담임={CHURCH['pastor']}
Five Ways: {', '.join(CHURCH['five_ways'])}

설교 정보: {info['n']}번, {info['worship']}, {info['date']}

아래 JSON 형식으로만 응답하세요 (코드펜스 없이):
...
{{
  ...
  "verse_ref": "책 장:절",
  "sections": [
    {{"label": "배경 · 도입", "paragraphs": ["문단1","문단2","문단3"], "verse_box": {{"text":"인용구절","ref":"출처"}}, "emphasis": "강조문장"}},
    {{"label": "1부 · 소제목", "paragraphs": ["문단1","문단2","문단3","문단4"], "verse_box": {{"text":"인용구절","ref":"출처"}}, "emphasis": "강조문장"}},
    {{"label": "2부 · 소제목", "paragraphs": ["문단1","문단2","문단3"], "verse_box": null, "emphasis": "강조문장"}},
    {{"label": "3부 · 소제목", "paragraphs": ["문단1","문단2","문단3"], "verse_box": {{"text":"인용구절","ref":"출처"}}, "emphasis": "강조문장"}},
    {{"label": "마무리 · 이번 주 실천", "paragraphs": ["문단1","문단2","문단3"], "verse_box": {{"text":"기도문","ref":"— 아멘 —"}}, "emphasis": "느리게 · 바르게 · 함께 · 아멘"}}
  ],
  "plain_text": "TTS용 전체 설교 (2000자 이상, 충분히 풍성하게)",
  "reading_minutes": 10
}}

요구사항: 20~40대 직장인 대상, 스마트폰 공감, Five Ways 하나 연결, 마지막은 반드시 느리게 바르게 함께 아멘으로 끝, 한국어만, JSON만 반환"""
...
def build_html(info, sermon):
    prev_n = info['n'] - 1
    mins = sermon.get("reading_minutes", 10)
    sections_html = ""

    for sec in sermon["sections"]:
        p_html = "\n".join(f"      <p>{p}</p>" for p in sec.get("paragraphs", []))
        vb = sec.get("verse_box")
        vb_html = f"""      <div class="verse-box"><div class="v-text">"{vb['text']}"</div><div class="v-ref">— {vb['ref']} —</div></div>""" if vb else ""
        em = sec.get("emphasis")
        if em:
            em_html = f'      <p style="font-family:var(--font-c);font-size:11px;letter-spacing:4px;color:var(--gold);opacity:.7;text-align:center;margin-top:24px">{em}</p>' if "아멘" in em else f'      <p><span class="emphasis">{em}</span></p>'
        else:
            em_html = ""
...
"""
    ...
    pe = sermon["plain_text"].replace("\\","\\\\").replace("`","\\`").replace("${","\\${")
...
<style>
:root{{--navy:#0B1622;--gold:#C9A84C;--gold-l:#E8C96A;--gold-p:#F0DFAA;--sage:#7A9E7E;--cream:#F5EDD6;--border:rgba(201,168,76,0.14);--font-s:'Noto Serif KR',serif;--font-c:'Cinzel',serif}}
...
.meta-item{{font-family:var(--font-c);font-size:9px;letter-spacing:3px;color:var(--cream);opacity:.4;text-transform:uppercase}}
...
.tts-settings{{display:flex;padding-top:14px;border-top:1px solid rgba(201,168,76,.1);margin-top:10px}}
...
<div class="hero">
  ...
  <div class="sermon-meta">
    ...
    <span class="meta-item">⏱ 낭독 약 {mins}분</span>
  </div>
  ...
  <div class="tts-player">
    <div class="tts-header">
      <div class="tts-info">
        ...
        <div class="tts-sub">낮고 차분한 목소리 · 약 {mins}분</div>
      </div>
    ...
    <div class="tts-time-row">
      ...
      <span class="tts-time">약 {mins}:00</span>
    </div>
...
window.addEventListener('load',()=>{{
  sermonAudio=document.getElementById('sermonAudio');document.getElementById('ttsStatusText').textContent='재생 준비 ✅';
...
function ttsPlay(){{sermonAudio.currentTime=0;sermonAudio.play().catch(e=>console.log(e));isPlaying=true;isPaused=false;document.getElementById('playBtn').textContent='⏸';document.getElementById('ttsDot').classList.add('on');document.getElementById('ttsStatusText').textContent='낭독 중 ✝';startTimer();}}
...
def update_sermons_data(info, sermon):
    ...
    content = Path(SERMONS_DATA_JS).read_text(encoding="utf-8") if Path(SERMONS_DATA_JS).exists() else ""
    new_entry = f"""  {{
    n: {info['n']},
    title: "{sermon['title']}",
    verse: "{sermon['verse']}",
    ...
    idx = content.rfind("}")
    content = (content[:idx+1] + ",\n" + new_entry + "\n\n];\n") if idx >= 0 else f"var SERMONS_DATA = [\n{new_entry}\n];\n"
...
def generate_sermon_tts(plain_text):
    ...
    try:
        ...
        print(f"  📄 {len(chunks)}파트 생성...")
        audio_parts = []
        for i, chunk in enumerate(chunks):
            resp = client.synthesize_speech(
                input=texttospeech.SynthesisInput(text=chunk),
                voice=texttospeech.VoiceSelectionParams(language_code="ko-KR", name=cfg["name"]),
                audio_config=texttospeech.AudioConfig(audio_encoding=texttospeech.AudioEncoding.MP3, speaking_rate=cfg["rate"])
            )
            audio_parts.append(resp.audio_content)
            print(f"    파트 {i+1}/{len(chunks)} ✅")
        combined = b"".join(audio_parts)
        Path("sermon.mp3").write_bytes(combined)
        print(f"  ✅ sermon.mp3 완료 ({len(combined)//1024}KB)")
        return True
...
def git_commit_push(info):
    ...
    if Path("sermon.mp3").exists(): files.append("sermon.mp3")
    try:
        ...
        subprocess.run(["git", "commit", "-m", f"✝ sermon-{info['n']} ({info['date']})"], check=True)
...
def main():
    ...
    print("  🤖 Gemma 4 로컬 | API 키 불필요 | 완전 무료")
    print("=" * 60)
    ...
    print(f"\n📅 sermon-{info['n']}.html | {info['date']} | {info['worship']}")

    sermon = generate_sermon_content(info)

    print(f"\n🖊 HTML 생성: {info['filename']}")
    Path(info["filename"]).write_text(build_html(info, sermon), encoding="utf-8")
    print(f"  ✅ 완료")
